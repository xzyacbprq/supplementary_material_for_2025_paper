...
class MSELoss(nn.Module):
      ...
      def forward(self,pred,label, **kwargs):
          return self.loss_weight * ((pred - label)**2).mean()
...
@MODELS.register_module()
class KLDistLoss(nn.Module):
      def __init__(self, loss_weight = 1.0, T = 2):
          super().__init__()
          self.loss_weight = loss_weight
          self.T = T
          
      def forward(self,pred,label, **kwargs):
          soft_label = F.softmax(label/ self.T, dim = 1)
          soft_pred  = F.softmax(pred/ self.T, dim = 1)
          return (self.loss_weight * self.T**2) * (soft_label * (soft_label.log() - soft_pred.log())).mean()
