...
class EncoderDecoder(BaseSegmentor):
    ...
    def __init__(self,
        ...
        if use_aux_head == 'dlv3':
            self.resize_ind  = 1
            self.resize_frac = 2
            out_ch = feat_channel[0]//self.resize_frac
        ...
        if use_aux_head in ['upn', 'sfm']:
            self.resize_ind  = 0
            #out_ch = feat_channel[0]//self.resize_frac
            self.conv1     = nn.Conv2d(feat_channel[0],feat_channel[0]//2,1)
            self.conv2     = nn.Conv2d(feat_channel[1],feat_channel[1]//2,1)
            self.conv3     = nn.Conv2d(feat_channel[2],feat_channel[2]//2,1)
            self.conv4     = nn.Conv2d(feat_channel[3],feat_channel[3]//2,1)
            ch = num_class + 1 + sum(feat_channel)//2
            self.sp_attn   = nn.Conv2d(ch, 1 ,1 )
        else:
            self.resize_ind  = 0
            self.resize_frac = 1
    ...
    def extract_feat(self, inputs: Tensor, data_samples : SampleList = None) -> List[Tensor]:
        ...
        if len(self.use_aux_head):
            ...
            _aux_x = [None,None,None]
            #_aux_x[0] = self.resize(aux_x[0], _size)
            _aux_x[1] = self.resize(aux_x[1], _size)
            _aux_x[2] = self.resize(aux_x[2], _size)
            
            y   = cat([_x0, _x1, _x2, _x3, _aux_x[1], _aux_x[2]], dim =1)
            ...
            if self.use_aux_head == 'dlv3': 
                ...
                x = [self.resize(cat([_x0, _aux_x[1]], dim = 1) * y_atn, x[0].shape[2:]),
                     self.resize(cat([_x1, _aux_x[2]], dim = 1) * y_atn, x[1].shape[2:]),
