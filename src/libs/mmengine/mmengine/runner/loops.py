...
class IterBasedTrainLoop(BaseLoop):
    ...
    def run_iter(self, data_batch: Sequence[dict]) -> None:
        ...
        # synchronization during gradient accumulation process.
        # outputs should be a dict of loss.
        outputs = self.runner.model.train_step(data_batch, optim_wrapper=self.runner.optim_wrapper, itr = self._iter)
        self.runner.call_hook(
            'after_train_iter',
        ...
        path = f'{self.output_dir}/losses.csv'
        keys = list(outputs.keys())
        vals = list(map(str,[outputs[key].detach().cpu().numpy() for key in keys]))
...
class ValLoop(BaseLoop):
    ...
    def run(self) -> dict:
        ...
        # compute metrics
        metrics = self.evaluator.evaluate(idx + 1)
