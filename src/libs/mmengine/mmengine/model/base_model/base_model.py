...
class BaseModel(BaseModule):
    ...
    def train_step(self, data: Union[dict, tuple, list],
                   optim_wrapper: OptimWrapper, itr: int = None) -> Dict[str, torch.Tensor]:
        ...
        with optim_wrapper.optim_context(self):
            ...
            for key in data['data_samples'].keys():
                data['data_samples'][key] = data['data_samples'][key].to(device)
            losses = self._run_forward(data, mode='loss')  # type: ignore
        old_loss = {}
        for key in losses.keys():
            if 'aux' in key and 'loss' in key:
                old_loss[key.replace('loss','')] = losses[key]
                losses[key] = losses[key]#/((itr+50)/50)
        #old_loss['aux_lr'] = ((itr+1)/25)
        losses.update(old_loss)
    ...
    def val_step(self, data: Union[tuple, dict, list]) -> list:
        ...
        device = 'cuda'
        with torch.no_grad():
            data['inputs'] = data['inputs'].float().to(device)
            for key in data['data_samples'].keys():
                data['data_samples'][key] = data['data_samples'][key].to(device)
            res = self._run_forward(data, mode='predict')  # type: ignore
        return res
