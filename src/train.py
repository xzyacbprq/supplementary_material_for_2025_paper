...
def train(model, 
    ...
    for epoch in range(epochs):
        ...
        for step,train_batch in tqdm(enumerate(train_loader), desc=f"Epoch {epoch + 1}/{epochs} - Training", leave=False):
            train_batch['inputs'] = train_batch['inputs'].float().to(device)
            for key in train_batch['data_samples'].keys():
                train_batch['data_samples'][key] = train_batch['data_samples'][key].to(device)
            loss_dict   = model.loss(train_batch['inputs'], train_batch['data_samples'])
        ...
        with torch.no_grad():
            for val_batch in tqdm(val_loader, desc="Validation", leave=False):
                ...
                for key in val_batch['data_samples'].keys():
                    val_batch['data_samples'][key] = val_batch['data_samples'][key].to(device)
